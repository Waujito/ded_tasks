...
int execv_gdb(pid_t program_pid, struct gdb_pipes pipes) {
	/*
	 * Redirects stdout to pipe.
	 */
	if (close(pipes.pipefd[0]) == -1)  /* Close unused read end */
		err(EXIT_FAILURE, "close");
	if (dup2(pipes.pipefd[1], STDOUT_FILENO) == -1)
		err(EXIT_FAILURE, "dup2");
	if (close(pipes.pipefd[1]) == -1)
		err(EXIT_FAILURE, "close");
	 ...
	 */
	if (close(pipes.pipefd_in[1]) == -1)
		err(EXIT_FAILURE, "close");
	if (dup2(pipes.pipefd_in[0], STDIN_FILENO) == -1)
		err(EXIT_FAILURE, "dup2");	
	if (close(pipes.pipefd_in[0]) == -1)
		err(EXIT_FAILURE, "close");
...
int gdb_running(pid_t program_pid, int signaling_fd) {
	...
	if (pipe2(pipes.pipefd, O_CLOEXEC)) {
		perror("pipe");
		exit(EXIT_FAILURE);
	}

	if (pipe2(pipes.pipefd_in, O_CLOEXEC)) {
		perror("pipe");
		exit(EXIT_FAILURE);
	}
	...
	if (forked_pid == 0) {
		if ((ret = execv_gdb(program_pid, pipes))) {
			return ret;
		}
	} else {
